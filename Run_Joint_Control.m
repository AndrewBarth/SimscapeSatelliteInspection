...
while time(t) <= tEnd
    ...
    [geometry(t)] = computeGeometry(x0(:,t),nLink,Link_Length,massVec,alpha,Base_a,Base_c,Base_z);

    
    [tau(:,t),linkInertia,qDDot(:,t),momentum(:,t)] = jointControl(x0(:,t),geometry(t),nLink,qCmdDot,qCmd,eeRateCmd,Kp,Kd,cntrlMode,Link_Length,massProperties(t));
    massProperties(t).linkInertia = linkInertia;
...
%     
%     [cstar] = computeCstar(geometry(t),massProperties(t),jAngle,qDot,Jt,Jr,Hom,HoInv,Link_Length,nLink);    
%     
%     qDDot(:,t) = HstarInv*(tau(:,t) + HstarDot*qDot' - cstar');  % Ref 1, Eq. 47
    
    % Update the state
    nState = size(x0,1);
    dx0(1:6+nLink,t+1) = x0(6+nLink+1:nState,t);
    dx0(6+nLink+1:nState,t+1) = [0 0 0 0 0 0 qDDot(:,t)'];
...
% Update the geometry with the final state vector
[geometry(t)] = computeGeometry(x0(:,t-1),nLink,Link_Length,massVec,alpha,Base_a,Base_c,Base_z);
